...
def analyze_and_generate_output(csv_file):
    ...
    recommended_columns_outliers  = column_selector(loadedcsv, prompt_outliers, num_columns=5, column_type="numeric")
    ...
    if img_cnt <= max_img:
        ...
        relevant_columns_heatmap = column_selector(loadedcsv, prompt_heatmap, num_columns=5, column_type="numeric")
    ...
    if img_cnt <= max_img:
        ...
        recommended_columns = []
        recommended_columns=column_selector(loadedcsv, prompt_clustering, num_columns=3, column_type="numeric")
        # Validate the recommended columns against the loaded CSV columns
        valid_recommended_columns = validate_columns(recommended_columns, columnslist)
        # Use the valid recommended columns for further analysis
        if valid_recommended_columns or len(valid_recommended_columns) > 0:
            if len(recommended_columns) >= 1:
                best_column1 = valid_recommended_columns[0]
            if len(recommended_columns) >= 2:
                best_column2 = valid_recommended_columns[1]
            if len(recommended_columns) >= 3:
                best_column3 = valid_recommended_columns[2]
        ...
        if not valid_recommended_columns or len(valid_recommended_columns) == 0:
    ...
    if img_cnt <= max_img:
        ...
        recommended_algorithm=clustering_algorithm_selector(loadedcsv, prompt_clustering_alg, selected_columns)
    ...
    if img_cnt <= max_img:
        ...
        relevant_categorical_columns = column_selector(loadedcsv, prompt_categorical, num_columns=1, column_type="categorical")
    ...
    if img_cnt <= max_img:
        ...
        relevant_columns = []
        relevant_columns = column_selector(loadedcsv, prompt_timeseries, num_columns=1, column_type="numeric")
    ...
    if img_cnt <= max_img:
        ...
        relevant_columns = []
        relevant_columns = recommended_columns
        # Execute Time Series Plot    
...
def standard_analysis_csv(loadedcsv, columnslist):
    ...
    relevant_columns = []
    relevant_columns = column_selector(loadedcsv, prompt, num_columns=None, column_type="numeric")
    if not relevant_columns or len(relevant_columns) == 0:
...
def plot_categorical_data(loadedcsv, output_path, relevant_categorical_columns):
    ...
    best_column = relevant_categorical_columns[0] if isinstance(relevant_categorical_columns, list) else relevant_categorical_columns 
    ...
    if best_column in non_numeric_df.columns:
        ...
        fig, axs = mathplt.subplots(2 if num_unique_values > 15 else 1, 1, figsize=(512/100, 512/100), dpi=100, squeeze=False)
        axs = axs.flatten()

        if num_unique_values <= 15:
            # Single plot for up to 15 unique values
            ax = axs[0]
            sns.countplot(x=best_column, data=loadedcsv, order=value_counts.index, ax=ax)
            ax.set_title(f"Distribution of {best_column}")
            ...
            axs[0].set_title(f"Top 15 Distribution of {best_column}")
            ...
            axs[1].set_title(f"Bottom 15 Distribution of {best_column}")
            adjust_labels(axs[1], bottom_15.index, max_chars_per_line=15, rotate=True)
       
        mathplt.tight_layout()
        consolidated_path = os.path.join(output_path, f"Categorical_Distribution_{best_column}.png")
        if img_cnt <= max_img:
            mathplt.savefig(consolidated_path, dpi=100)
            img_cnt += 1
        mathplt.close()
...
def validate_columns(recommended_columns, loadedcsv_columns):
  """
  Validates if the recommended columns are present in the loaded CSV columns.

  Args:
    recommended_columns: A list of recommended column names.
    loadedcsv_columns: A list of column names in the loaded CSV.

  Returns:
    A list of valid recommended columns.
  """
  valid_columns = [col for col in recommended_columns if col in loadedcsv_columns]
...
def column_selector(loadedcsv, inputprompt, num_columns=None, column_type="all"):
    ...
    if not isinstance(loadedcsv, pd.DataFrame):
        raise ValueError("loadedcsv must be a pandas DataFrame.")
    if not isinstance(inputprompt, str):
        raise ValueError("inputprompt must be a string.")
    if num_columns is not None and (not isinstance(num_columns, int) or num_columns <= 0):
        raise ValueError("num_columns must be a positive integer or None.")
    if column_type not in {"all", "numeric", "categorical"}:
        raise ValueError("column_type must be 'all', 'numeric', or 'categorical'.")

    if API_KEY:
        # Prepare the prompt
        prompt = inputprompt.format(columns=loadedcsv.columns.tolist())
        if num_columns is not None:
            prompt += f"\nPlease provide exactly {num_columns} column names from the dataset."
        if column_type != "all":
            prompt += f"\nFocus only on {column_type} columns."
        prompt += "\nReturn the column names as a plain, comma-separated list (e.g., column1, column2)."
        ...
        try:
            ...
            recommended_columns = [col.strip() for col in raw_content.split(",") if col.strip() in loadedcsv.columns]
